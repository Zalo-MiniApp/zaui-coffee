...
const CategoryPicker: FC = () => {
  ...
  return (
    <Tabs scrollable defaultActiveKey={selectedCategory} className="category-tabs">
      {categories.map(category => <Tabs.Tab
        key={category.id}
        label={category.name}
      >
        <Suspense>
...
const CategoryProducts: FC<{ categoryId: string }> = ({ categoryId }) => {
  ...
  if (productsByCategory.length === 0) {
    return <Box className="flex-1 bg-background p-4 flex justify-center items-center">
      <Text size="xSmall" className="text-gray">Không có sản phẩm trong danh mục</Text>
    </Box>
  }
  return (
